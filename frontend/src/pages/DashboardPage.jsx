...
import {
  ...
  useDisclosure, Table, Thead, Tbody, Tr, Th, Td
...
const STATUS_ORDER_WEIGHT = { 'Done': 1, 'To Do': 2, 'Need Revision': 2, 'Reviewed': 3 };
...
function DashboardPage({ user, usersList, onLogout }) { 
  ...
  let filteredTasks = tasks.filter(task => {
    ...
    // PERBAIKAN: task.assignee_id jadi task.user_id
    if (filterStatus === 'my_tasks') return task.user_id === user?.user_id;
    ...
    <Box p={5} maxWidth="1200px" margin="auto">
      ...
      <Tabs isFitted variant='enclosed' colorScheme='teal' isLazy>
        ...
        <TabPanels>
          {user?.role !== 'admin' && (
            <TabPanel p={0}>
              <Flex mb={4} gap={4} flexWrap="wrap" alignItems="center">
                  ...
                  {/* Tombol Tambah Tugas Eksklusif Team Lead Mentok Kanan */}
                  {user?.role === 'team lead' && (
                      ...
                      {filteredTasks.length === 0 ? (
                        ...
                        filteredTasks.map((task) => (
                          <Tr key={task.task_id} _hover={{ bg: "gray.50" }}>
                            ...
                            <Td>
                               {/* PERBAIKAN: task.assignee jadi task.user */}
                               {task.user ? <Badge colorScheme="purple" variant="subtle" px={2} borderRadius="full">{task.user.nama}</Badge> : <Text fontSize="sm" color="gray.400">Unassigned</Text>}
                            </Td>
                            <Td>
                            ...
                            <Td textAlign="center">
                              {/* PERBAIKAN: task.assignee_id jadi task.user_id */}
                              {user?.role === 'intern' && task.user_id === user?.user_id && (task.status === 'To Do' || task.status === 'Need Revision') && (
          ...
          {user?.role === 'admin' && (
            <TabPanel p={0}><UserManagement users={usersList} /></TabPanel>
          )}
        </TabPanels>
      </Tabs>

      <AddTaskForm isOpen={addTaskDisclosure.isOpen} onClose={addTaskDisclosure.onClose} users={usersList ? usersList.filter(u => u.role === 'intern') : []} onTaskAdded={addTaskLocally} />
